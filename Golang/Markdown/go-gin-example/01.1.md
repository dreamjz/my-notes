...
在`models/request`目录下创建`common.go`
...
在`models/response`目录下新建`response.go`
...
在`model/response`下创建通用的分页响应数据结构`common.go`：
...
```sh
go get github.com/spf13/cast@latest
```
...
func InitArticleRouter(group *gin.RouterGroup) {
	tagGroup := group.Group("tag")
	{
		...
		tagGroup.PUT("/edit", v1.EditTagById)
		tagGroup.DELETE("/delete", v1.DeleteTagById)
	}
}
```
...
在`dao`目录下添加数据库访问逻辑，创建`tag.go`写入：
...
在`api/vi/tag.go`中添加获取标签列表的业务逻辑：
...
在`routers`下创建`article.go`，新增`articles`路由组：
...
在`models/request`下创建`article.go`:
